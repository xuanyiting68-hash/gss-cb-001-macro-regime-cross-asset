...
OUT = ROOT / "results" / "fed_cycle_path_v1"
...
RECENT_SCHEDULED_DECISIONS = [
    # Federal Reserve FOMC calendars; only the recent supplement needed
    # to complete the 2022-24 tightening/pause/easing sequence.
...
def nearest_scheduled_flag(dt: pd.Timestamp, scheduled: pd.DatetimeIndex, tol_days=1):
    ...
    return bool(np.min(diffs) <= tol_days)
...
def build_cycles(changes: pd.DataFrame, scheduled: pd.DatetimeIndex):
    ...
    for run_id, g, cum_bp in positive_runs:
        cycle_no += 1
        cycle_id = f"T{cycle_no:02d}_{g['date'].iloc[0].year}"
        first = g.iloc[0]
        last = g.iloc[-1]
        later = changes[changes["date"] > last["date"]]
        next_cuts = later[later["change_bp"] < 0]
        first_cut = next_cuts.iloc[0] if len(next_cuts) else None

        pause_start = pd.NaT
        if first_cut is not None:
            candidates = scheduled[(scheduled > last["date"]) & (scheduled < first_cut["date"])]
            if len(candidates):
                pause_start = candidates[0]

        cycle_rows.append({
            "cycle_id": cycle_id,
            "first_hike": first["date"],
            "last_hike": last["date"],
            "pause_start": pause_start,
            "first_cut": first_cut["date"] if first_cut is not None else pd.NaT,
            "n_hikes": int(len(g)),
        ...
        def add_event(event_type, row=None, date=None, change_bp=np.nan):
            ...
            event_rows.append({
                ...
                "target_after": after,
                "change_bp": cbp,
                "scheduled_decision_match": nearest_scheduled_flag(pd.Timestamp(dt), scheduled),
                "cycle_start_year": int(first["date"].year),
            })
    ...
    # Easing legs: descriptive ontology, not attached to a tightening cycle if unmatched.
    easing_no = 0
    for run_id, g in changes.groupby("run_id"):
        if int(g["sign"].iloc[0]) != -1:
            continue
        easing_no += 1
        e_id = f"E{easing_no:02d}_{g['date'].iloc[0].year}"
        for etype, row in [("EASING_CYCLE_START", g.iloc[0]), ("EASING_CYCLE_END", g.iloc[-1])]:
            event_rows.append({
                ...
                "scheduled_decision_match": nearest_scheduled_flag(row["date"], scheduled),
                "cycle_start_year": int(g["date"].iloc[0].year),
            })

    # Emergency cuts are mechanically unscheduled cuts >=25bp.
    for _, row in changes[changes["change_bp"] <= -25.0].iterrows():
        if not nearest_scheduled_flag(row["date"], scheduled):
            event_rows.append({
                "event_id": f"EMERGENCY_{row['date'].date()}",
                "cycle_id": "",
                "event_type": "EMERGENCY_CUT",
                "event_date": row["date"],
                "target_before": row["target_before"],
                "target_after": row["target"],
                "change_bp": row["change_bp"],
                "scheduled_decision_match": False,
                "cycle_start_year": int(row["date"].year),
            })
...
def write_report(cycles, events, metrics, summary, key_gold, qc, coverage):
    ...
    lines = [
        ...
        f"- Emergency-cut rows: {int((events['event_type'] == 'EMERGENCY_CUT').sum())}",
    ...
    (OUT / "FED_CYCLE_PATH_001_REPORT.md").write_text("\n".join(lines), encoding="utf-8")
...
def main():
    ...
    cycles, events = build_cycles(changes, scheduled)

    # Freeze outcome-relevant event set to primary tightening anchors plus emergency cuts.
    primary_types = {
        "FIRST_HIKE","LAST_HIKE","PAUSE_START","FIRST_CUT",
        "TIGHTENING_CYCLE_START","TIGHTENING_CYCLE_END","EMERGENCY_CUT",
    }
    outcome_events = events[events["event_type"].isin(primary_types)].copy()
    ...
    qc = {
        ...
        "qualifying_tightening_cycles": int(len(cycles)),
    ...
    changes[["date","target_before","target","change_bp","direction"]].to_csv(
        OUT / "FED_POLICY_CHANGE_REGISTRY.csv", index=False
    )
    ...
    if hard_fail:
        raise SystemExit("FED-CYCLE-PATH-001 QC gate failed")

    print(json.dumps({
        "qc_gate": qc["qc_gate"],
        "qualifying_cycles": qc["qualifying_tightening_cycles"],
