...
OUT=ROOT/"results"/"fed_cycle_listed_reit_proxy_bridge_036_v1"
...
def main():
    ...
    if set(srcspec["symbol"])!={"VNQ","VGSIX"}:
    ...
    if vfirst>pd.Timestamp("1996-06-30"):
        raise RuntimeError(f"VGSIX history begins too late: {vfirst}")
    ...
    tlt=monthly["VNQ"].set_index("period")["value"].astype(float)
    vus=monthly[PROXY_ID].set_index("period")["value"].astype(float)
    tr=tlt.pct_change(fill_method=None).rename("vnq_monthly_ret")
    vr=vus.pct_change(fill_method=None).rename("vgsix_monthly_ret")
    ov=pd.concat([tr,vr],axis=1,join="inner").dropna()
    if ov.empty:
        raise RuntimeError("no monthly overlap")
    pearson=float(ov["vnq_monthly_ret"].corr(ov["vgsix_monthly_ret"],method="pearson"))
    spearman=float(ov["vnq_monthly_ret"].corr(ov["vgsix_monthly_ret"],method="spearman"))
    beta,intercept=np.polyfit(ov["vnq_monthly_ret"].to_numpy(),ov["vgsix_monthly_ret"].to_numpy(),1)
    ov["abs_return_diff"]=abs(ov["vgsix_monthly_ret"]-ov["vnq_monthly_ret"])
    ...
    overlap_diag=pd.DataFrame([{
        ...
        "descriptive_beta_vgsix_on_vnq":float(beta),
    ...
    assets={
        "VNQ":{"df":monthly["VNQ"],"label":"VNQ target","source":"fresh Yahoo adjusted close"},
        PROXY_ID:{"df":monthly[PROXY_ID],"label":"VGSIX listed-REIT proxy","source":"fresh Yahoo adjusted close"}
    }
    met,_=EXP.build_market(cycles,assets)
    common=met[met["asset"]=="VNQ"].merge(
        met[met["asset"]==PROXY_ID],
        on=["anchor","cycle_id","broad_episode_id","anchor_date"],
        suffixes=("_vnq","_vgsix"),
        validate="one_to_one"
    )
    if common.empty:
    ...
    common["ret12_diff"]=common["ret_12m_vgsix"]-common["ret_12m_vnq"]
    common["abs_ret12_diff"]=abs(common["ret12_diff"])
    common["mdd_diff"]=common["mdd_12m_vgsix"]-common["mdd_12m_vnq"]
    common["abs_mdd_diff"]=abs(common["mdd_diff"])
    common["ret12_sign_agree"]=(np.sign(common["ret_12m_vgsix"])==np.sign(common["ret_12m_vnq"])).astype(float)
    common.to_csv(OUT/"COMMON_EVENT_BRIDGE_PANEL.csv",index=False)

    event_sign_agree=float(common["ret12_sign_agree"].mean())
    event_ret_corr=float(common["ret_12m_vnq"].corr(common["ret_12m_vgsix"]))
    event_abs_ret_med=float(common["abs_ret12_diff"].median())
    mdd_corr=float(common["mdd_12m_vnq"].corr(common["mdd_12m_vgsix"]))
    ...
    for anchor in PHASES:
        ...
        tlt12=weighted_median(g["ret_12m_vnq"],w)
        vus12=weighted_median(g["ret_12m_vgsix"],w)
        tltmdd=weighted_median(g["mdd_12m_vnq"],w)
        vusmdd=weighted_median(g["mdd_12m_vgsix"],w)
        ps.append({
            "anchor":anchor,"n_common_events":len(g),"n_common_broad_episodes":g["broad_episode_id"].nunique(),
            "vnq_weighted_median_ret_12m":tlt12,
            "vgsix_weighted_median_ret_12m":vus12,
            "ret12_abs_diff":abs(vus12-tlt12),
            "vnq_direction":direction(tlt12),"vgsix_direction":direction(vus12),
            "direction_agree":direction(tlt12)==direction(vus12),
            "vnq_weighted_median_mdd_12m":tltmdd,
            "vgsix_weighted_median_mdd_12m":vusmdd,
    ...
    # Full VGSIX history metrics; never label as VNQ.
    ...
    proxy_met.to_csv(OUT/"VGSIX_EXTENDED_PHASE_METRICS.csv",index=False)
    ...
    ext_sum.to_csv(OUT/"VGSIX_EXTENDED_PHASE_SUMMARY.csv",index=False)
    ...
    d24.to_csv(OUT/"VGSIX_24M_DIAGNOSTICS.csv",index=False)
    ...
    qrows=[
        ("REIT-Q01","Did the proxy bridge pass?",bridge_status,"BRIDGE"),
        ("REIT-Q02","How similar are monthly VNQ and VGSIX returns?",f"Matched months {len(ov)}; Pearson {pearson:.3f}; Spearman {spearman:.3f}; beta {beta:.3f}; monthly intercept {intercept:+.5f}.","BRIDGE"),
        ("REIT-Q03","Do common Fed-cycle event outcomes agree?",f"12M event sign agreement {event_sign_agree:.0%}; return correlation {event_ret_corr:.3f}; median absolute 12M-return difference {event_abs_ret_med*100:.2f}pp; MDD correlation {mdd_corr:.3f}.","BRIDGE"),
        ("REIT-Q04","What does the longer proxy show at FIRST_HIKE?",f"VGSIX proxy 12M median {pp(sidx.loc['FIRST_HIKE','weighted_median_ret_12m'])}; MDD {sidx.loc['FIRST_HIKE','weighted_median_mdd_12m']*100:.1f}%; trough {mm(sidx.loc['FIRST_HIKE','weighted_median_mdd_trough_month'])}; support {sidx.loc['FIRST_HIKE','support_status']}.","PHASE"),
    ...
    if bridge_pass:
        narrative=[
            ...
            f"**{bridge_status}**",
            "",
            f"VNQ与VGSIX在完整重叠月的月收益相关性：Pearson {pearson:.3f}，Spearman {spearman:.3f}；描述性beta {beta:.3f}，intercept {intercept:+.5f}。",
            "",
    ...
    schema={
        "module":"FED-CYCLE-LISTED-REIT-PROXY-BRIDGE-036",
        "bridge_status":bridge_status,
    ...
    (OUT/"PANDAAI_REIT_SCHEMA.json").write_text(json.dumps(schema,indent=2,ensure_ascii=False)+"\n")

    report=[
        "# FED-CYCLE-LISTED-REIT-PROXY-BRIDGE-036 — REPORT",
        "",
        "## Status",
        "",
    ...
    if bridge_pass and (ext_sum["support_status"]=="NOT_PROMOTED_BRIDGE_FAIL").any():
